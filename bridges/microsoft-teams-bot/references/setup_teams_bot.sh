...
echo "     - Create DNS CNAME: <tunnel-hostname> -> <tunnel-id>.cfargotunnel.com (proxied)"
echo "  2. Update the Azure Bot messaging endpoint:"
echo "     az bot update --resource-group <rg> --name <bot> --endpoint https://<tunnel-hostname>/api/messages"
echo "  3. cp .env.example .env and paste the values above"
echo "  4. Set TENDRIL_INSTALL_KEY and CLOUDFLARE_TUNNEL_TOKEN in .env"
echo "  5. docker compose up -d --build"
echo "  6. SECURITY: Restrict webhook access at your CDN/reverse proxy edge."
echo "     If using Cloudflare, create two WAF rules (see SKILL.md 'Webhook Endpoint Security'):"
echo "     - ALLOW: POST /api/messages from ASN 8075 (Microsoft) with Authorization header"
echo "     - BLOCK: All other traffic to the bot hostname"
echo "  7. Tenant-wide deployment (Teams Admin PowerShell):"
echo "     a. Upload app manifest: New-TeamsApp -DistributionMethod organization -Path <manifest.zip>"
echo "     b. Fix permission policy: Set-CsTeamsAppPermissionPolicy -Identity Global -PrivateCatalogAppsType BlockedAppList"
echo "     c. Unblock app: Update-M365TeamsApp -Id <catalog-id> -IsBlocked \$false"
echo "     d. Pre-install globally: Set-CsTeamsAppSetupPolicy -Identity Global -AppPresetList @{Add=@(\$app)}"
echo "  8. For immediate user install: add TeamsAppInstallation.ReadWriteForUser.All permission"
echo "     and use Graph API POST /users/{id}/teamwork/installedApps"
echo ""
echo "NOTE: Teams admin policy changes can take 1-24 hours to propagate."
echo "      New Graph API permissions can take 15-60 minutes to appear in tokens."
echo ""
